package org.sobadfish.hunger.room;
...
import org.sobadfish.hunger.event.GameCloseEvent;
import org.sobadfish.hunger.event.GameRoomStartEvent;
import org.sobadfish.hunger.event.PlayerJoinRoomEvent;
import org.sobadfish.hunger.event.PlayerQuitRoomEvent;
import org.sobadfish.hunger.item.button.FollowItem;
import org.sobadfish.hunger.item.button.RoomQuitItem;
import org.sobadfish.hunger.manager.RandomJoinManager;
import org.sobadfish.hunger.manager.RoomManager;
...
public class GameRoom {
     ...
     * 传入胜利的队伍
     * */
    public void gameEnd(PlayerInfo teamInfo){
        teamInfo.getTeamInfo().echoVictory(teamInfo);
        teamInfo.getTeamInfo().echoDefeat();
        type = GameType.END;
    ...
    private void onStart() {
        ...
        if(loadTime > 0) {
            ...
            //TODO 在房间倒计时结束
            double heath = 0;
            TeamInfo successInfo = getTeamInfos().get(0);
            PlayerInfo info = null;
            for(PlayerInfo playerInfo:successInfo.getLivePlayer()){
                if(playerInfo.getPlayer().getHealth() > heath){
                    info = playerInfo;
    ...
    private void onWait() {
        ...
        if(loadTime >= 1) {
            ...
            //sendActionBar("&e距离开始还剩 &a " + loadTime + " &e秒");
            //sendActionBar("&a距离开始游戏还剩 &l&c " + loadTime);
            if(loadTime <= 5){
                /*switch (loadTime){
